...
本文檔整理 Eigent 專案中所有 Agent 的提示詞（Prompt）設定位置與內容。

## 目錄

- [Agent 列表](#agent-列表)
- [Prompt 設定位置](#prompt-設定位置)
- [各 Agent 提示詞內容](#各-agent-提示詞內容)
...
### 核心協調 Agent
1. **task_agent** - 任務規劃 Agent
2. **coordinator_agent** - 協調者 Agent
3. **new_worker_agent** - 新工作 Agent

### 專業功能 Agent
4. **developer_agent** - 開發者 Agent
5. **browser_agent** - 瀏覽器 Agent
6. **document_agent** - 文件 Agent
7. **multi_modal_agent** - 多模態 Agent
8. **social_medium_agent** - 社群媒體 Agent (註解掉)
9. **mcp_agent** - MCP Agent (不可見)

### 輔助 Agent
10. **question_confirm_agent** - 問題確認 Agent
11. **task_summary_agent** - 任務摘要 Agent
...
### 儲存方式
**所有 Agent 的 Prompt 均使用字串（String）形式定義在程式碼中，未使用外部檔案。**

### 檔案位置

| Agent | 檔案位置 | 函數/位置 |
|-------|---------|----------|
| `task_agent` | `backend/app/service/chat_service.py` | `construct_workforce()` (line 1330-1335) |
| `coordinator_agent` | `backend/app/service/chat_service.py` | `construct_workforce()` (line 1320-1329) |
| `new_worker_agent` | `backend/app/service/chat_service.py` | `construct_workforce()` (line 1338-1345) |
| `developer_agent` | `backend/app/utils/agent.py` | `developer_agent()` (line 779-965) |
| `browser_agent` | `backend/app/utils/agent.py` | `browser_agent()` (line 969-1166) |
| `document_agent` | `backend/app/utils/agent.py` | `document_agent()` (line 1170-1389) |
| `multi_modal_agent` | `backend/app/utils/agent.py` | `multi_modal_agent()` (line 1393-1582) |
| `social_medium_agent` | `backend/app/utils/agent.py` | `social_medium_agent()` (line 1586-1711) |
| `mcp_agent` | `backend/app/utils/agent.py` | `mcp_agent()` (line 1715-1786) |
| `question_confirm_agent` | `backend/app/utils/agent.py` | `question_confirm_agent()` (line 761-766) |
| `task_summary_agent` | `backend/app/utils/agent.py` | `task_summary_agent()` (line 770-775) |

### 自定義 Agent
當使用者建立自定義 Agent 時，Prompt 會在 `backend/app/service/chat_service.py` 的 `new_agent_model()` 函數中動態生成（line 1457-1480）。
...
**位置**: `backend/app/service/chat_service.py:1330-1335`
...
**位置**: `backend/app/service/chat_service.py:1320-1329`
...
**位置**: `backend/app/service/chat_service.py:1338-1345`
...
**位置**: `backend/app/utils/agent.py:817-949`
...
**位置**: `backend/app/utils/agent.py:1036-1146`
...
**位置**: `backend/app/utils/agent.py:1218-1369`
...
**位置**: `backend/app/utils/agent.py:1477-1562`
...
### 8. social_medium_agent (社群媒體 Agent)

**位置**: `backend/app/utils/agent.py:1626-1695`
...
### 9. mcp_agent (MCP Agent)

**位置**: `backend/app/utils/agent.py:1764`
...
**位置**: `backend/app/utils/agent.py:764`
...
**位置**: `backend/app/utils/agent.py:773`
...
當使用者建立自定義 Agent 時，系統會在 `backend/app/service/chat_service.py` 的 `new_agent_model()` 函數中動態生成 Prompt：

**位置**: `backend/app/service/chat_service.py:1473-1478`
...
自定義 Agent 的 Prompt 由使用者提供的 `description` 加上系統環境資訊組成。

---

## 總結

- **總 Agent 數量**: 11 個（9 個核心 Agent + 2 個輔助 Agent）
- **Prompt 儲存方式**: 全部使用字串（String），定義在 Python 程式碼中
- **主要檔案**: 
  - `backend/app/utils/agent.py` - 包含大部分 Agent 的 Prompt
  - `backend/app/service/chat_service.py` - 包含協調類 Agent 和自定義 Agent 的 Prompt 生成邏輯
